...
<div class="content">
    ...
    @endif
    <form id="DemandNoteForm" method="post" action="{{route('demandnote.restore')}}" enctype="multipart/form-data">
        @csrf
        <div class="card">
            <div class="card-body">
                    ...
                    <div class="table-responsive mb-3">
                        <table class="table">
                            ...
                            <tbody id="tbodyProductList">
                                @if(isset($productBOM) && !empty($productBOM))
                                    @php $i=1; @endphp
                                    @foreach($productBOM as $bom1)
                                    <tr>
                                        <td>{{$i}}</td>
                                        <td>{{RawMaterialName($bom1['id'])->name}}</td>
                                        <td>{{RawMaterialName($bom1['id'])->code}}</td>
                                        <td><div class="form-group" style="margin-bottom: 0px;"><input type="number" style="padding:5px 5px" name="dnote[{{$bom1['id']}}][demandQun]" min="1" class="txtqun"></div></td>
                                ...
                                <input type="hidden" name="QCId" value="{{$ProductQCData->id}}">
                            </tbody>
                        </table>
                    </div>
                </div>
