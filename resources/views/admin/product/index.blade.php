...
<div class="content">
    <div class="page-header">
        ...
        <div class="page-btn">
            <a href="{{route('product.create')}}" class="btn btn-added"><img src="{{URL::asset('admin_asset/img/icons/plus.svg')}}" alt="img" class="me-1">Add New Product</a>
        </div>
    </div>
    <div class="card">
        <div class="card-body">
            ...
            <table class="table datanew">
                ...
                @foreach($Product as $k=>$values)
                    <tr id="id_rmv{{$values->id}}">
                        ...
                        <td>
                            <a class="me-3" href="{{route('product.show',$values->id)}}">
                            <img src="{{URL::asset('admin_asset/img/icons/eye.svg')}}" alt="img">
                            </a>
                            <a class="me-3" href="{{route('product.edit',$values->id)}}">
...
$(".Delete").click(function() {
    removethis($(this).data('id'),"{{url('product_destroy')}}","{{ csrf_token() }}");
});
$('.statuschange').change(function() {
    changeAllStatus($(this).val(),"{{route('product.status')}}","{{ csrf_token() }}");
});
